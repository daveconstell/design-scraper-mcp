...
class FontAnalyzer {
    ...
    const fontData = await this.page.evaluate(() => {
      ...
      elements.forEach(el => {
        ...
        if (fontFamily && fontFamily !== 'inherit') {
          ...
          const isBody =
            bodySelectors.includes(tagName) ||
  ...
  async analyzeFonts(url) {
    try {
      ...
      const fontFamilies = Array.from(uniqueFontFamilies.values()).sort(
    ...
    const result = {
      headings:
        analysis.headings.length > 0
          ? analysis.headings[0].fontFamily
          : 'Not detected',
      body:
        analysis.body.length > 0 ? analysis.body[0].fontFamily : 'Not detected',
    };
