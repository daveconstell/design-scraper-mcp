...
class BorderAnalyzer {
  ...
  async extractBorders(url) {
    ...
    const borderData = await this.page.evaluate(() => {
      ...
      elements.forEach(el => {
        ...
        if (border && border !== 'none' && border !== '0px none rgba(0, 0, 0, 0)') {
      ...
      return {
        borderRadius: Array.from(borderUsage.borderRadius.entries())
          .sort((a, b) => b[1] - a[1]),
        borderWidth: Array.from(borderUsage.borderWidth.entries())
          .sort((a, b) => b[1] - a[1]),
        borderColor: Array.from(borderUsage.borderColor.entries())
          .sort((a, b) => b[1] - a[1])
      };
    });
  ...
  async analyzeBorders(url) {
    try {
      ...
      const mostCommonBorderRadius = borderData.borderRadius.length > 0 
        ? this.parseBorderRadius(borderData.borderRadius[0][0]) 
        : null;
      
      const mostCommonBorderWidth = borderData.borderWidth.length > 0 
        ? this.parseBorderWidth(borderData.borderWidth[0][0]) 
        : null;
      
      const mostCommonBorderColor = borderData.borderColor.length > 0 
