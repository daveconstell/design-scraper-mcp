...
class ColorPrinter {
  ...
  createColorSwatch(hex, label = '', width = 20) {
    ...
    return chalk
      .rgb(contrastRgb.r, contrastRgb.g, contrastRgb.b)
  ...
  printElementColors(elements) {
    ...
    if (
      elements.body &&
      (elements.body.background || elements.body.foreground)
    ) {
      this.printColorCombination(
        elements.body.background,
        elements.body.foreground,
        'Body Content'
      );
    }
    ...
    if (
      elements.header &&
      (elements.header.background || elements.header.foreground)
    ) {
      this.printColorCombination(
        elements.header.background,
        elements.header.foreground,
        'Header Content'
      );
    }
    ...
    if (
      elements.footer &&
      (elements.footer.background || elements.footer.foreground)
    ) {
      this.printColorCombination(
        elements.footer.background,
        elements.footer.foreground,
        'Footer Content'
      );
    }
  ...
  printColorGrid(colors, title = 'Colors', columns = 4) {
    ...
    for (let i = 0; i < colors.length; i += columns) {
      ...
      const swatches = row
        .map(color => (color ? this.createColorSwatch(color, '', 15) : ''))
        .join(' ');
    }
  }
}
